...
class SeldModel(BaseModel):
    ...
    def forward(self, x):
        ...
        # print(f'event_frame_logit.shape={output_dict["event_frame_logit"].shape}')
        # print(f'doa_frame_output.shape={output_dict["doa_frame_output"].shape}')
        # output_dict = {
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        #     B, T, C3 = pred_dict['doa_frame_output'].shape
        #     pred_dict['doa_frame_output'] = pred_dict['doa_frame_output'].reshape(B, T, 3, C3//3)
        #     denom = torch.sqrt(pred_dict['doa_frame_output'][:,:,0,:]**2 + pred_dict['doa_frame_output'][:,:,1,:]**2 + pred_dict['doa_frame_output'][:,:,2,:]**2)
        #     denom = denom.unsqueeze(dim=2)
        #     pred_dict['doa_frame_output'] = pred_dict['doa_frame_output'] / denom
        #     pred_dict['doa_frame_output'] = pred_dict['doa_frame_output'].reshape(B, T, C3)
        # write output file
